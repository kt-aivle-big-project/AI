        ...
        *,
        ...
        if g2p_mode:
            ...
            for station in self.repository.outbound_station_candidates(logical_destinations):
                ...
                for destination_id in logical_destinations:
                    add_relation(
        ...
        if g2p_mode:
            ...
            station_accesses = {
                str(station["station_id"]): [
                    str(value) for value in station.get("access_node_ids", [])
                ]
            ...
            for stock in positive_stocks:
                for source_access in stock.access_node_ids:
                    for station_id, access_ids in station_accesses.items():
                        for station_access in access_ids:
                            _, arcs = graph.shortest_path(
                                source_access,
                                station_access,
                                metric="travel_time",
                            )
                            if arcs or source_access == station_access:
                                path = self._path_evidence(
                                    path_id=(
                                        f"path:g2p:{stock.stock_id}:{source_access}:"
                                        f"to:{station_id}:{station_access}"
                                    ),
                                ...
                                add_relation(
                                    ...
                                    f"station:{station_id}",
                                    "ROUTES_THROUGH",
                                    attributes={"station_access_node": station_access},
                                    evidence_ids=nodes[f"path_option:{path.path_id}"].evidence_ids,
                                )
                            ...
                            if return_arcs or station_access == source_access:
                                return_path = self._path_evidence(
                                    path_id=(
                                        f"path:g2p:{station_id}:{station_access}:"
                    ...
                    for station_id, access_ids in station_accesses.items():
                        for station_access in access_ids:
                            for buffer_access in empty_buffer_accesses:
                                ...
                                if buffer_arcs or station_access == buffer_access:
                                    register_path(
                                        self._path_evidence(
                                            path_id=path_id,
                                            purpose="STATION_TO_POST_MOVE",
