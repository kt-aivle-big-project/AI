...
class SimulationPlanBuilder:
    ...
    def build(
        ...
        *,
        ...
        repository = get_repository(warehouse_id, result.simulation_id)
        robot_start = {
                ...
                logical.append(
                    SimulationLogicalOperation(
                        ...
                        logical_destination_id=(
                            ...
                            if batch else order_need.delivery_node if order_need else None
                        ),
...
class RuntimeExecutionSnapshotBuilder:
    ...
    def _task_base(task_id: str | None) -> str | None:
        if not task_id:
            return None
        for suffix in ("_EMPTY_TOTE", "_RETURN", "_DROP", "_PICK"):
    ...
    def build(
        ...
        repository = get_repository(plan.warehouse_id, plan.simulation_id)
...
class RollingHorizonReplanService:
    ...
    def _merge_runtime_overrides(
        ...
        robot_by_id = {value.robot_id: value for value in snapshot.robot_overrides}
        robot_by_id.update({value.robot_id: value for value in explicit.robot_states})
        deviated_robot_ids = {
            value.robot_id
    ...
    def replan(self, request: ReplanMissionRequest) -> SimulationPlanResponse:
        ...
        if self.runner is not None:
            ...
            result = OrchestrationService().run(combined)

        plan = self.builder.build(
            result,
            plan_version=active.plan_version + 1,
            base_plan_id=active.plan_id,
            plan_start_sim_time_ms=request.replan_at_sim_time_ms,
            effective_from_sim_time_ms=request.replan_at_sim_time_ms,
        ...
        if plan:
            ReplanTimelineValidator.validate(
                plan, request.replan_at_sim_time_ms
            )
