    ...
    def run(
        ...
        *,
        ...
        with repository_scope(request.warehouse_id, request.simulation_id):
            base_repository = repository or create_request_repository(
                request.warehouse_id,
                request.simulation_id,
            )
            request_repository = (
                RequestOperationRepository(base_repository, request.structured_input)
            ...
            with repository_instance_scope(request_repository):
                ...
                "metadata": {
                    ...
                    "structured_operation_ids": (
                        [value.operation_id for value in request.structured_input.operations]
                        if request.structured_input is not None
                        else []
                    ),
