...
def _objective_terms_for_profile(objective_profile: str) -> list[str]:
    """Return the default auditable term for one solver objective profile."""

    mapped = {
        "MIN_TOTAL_COST": "MIN_TRAVEL_DISTANCE",
    ...
    return [mapped[objective_profile]]
...
def _objective_terms(request: NormalizedWarehouseRequest) -> list[str]:
    ...
    if explicit:
        return explicit
    return _objective_terms_for_profile(request.constraints.objective_profile)


def _objective_is_explicit(request: NormalizedWarehouseRequest) -> bool:
    """Return whether operator/request policy fixes the objective before formulation."""

    return bool(
        request.constraints.objective_profile_explicit
        or request.constraints.objective_terms
    )
        ...
        *,
        ...
        for relation in relations:
            if relation.relation_type == "HAS_ACCESS_POINT" and relation.source_node_id.startswith("station:"):
                ...
                relation.relation_type == "ROUTES_THROUGH"
                and relation.source_node_id.startswith("path_option:")
                and relation.target_node_id.startswith("station:")
            ):
                station_by_path[
                    relation.source_node_id.removeprefix("path_option:")
                ] = relation.target_node_id.removeprefix("station:")
                    ...
                    if g2p_mode:
                        delivery_paths = [
                            ...
                            if destination in destinations_by_station.get(
                                station_by_path.get(
                                    path.path_id,
                                    station_by_access.get(path.target_node_id, ""),
                                ),
                                set(),
                            )
                        ]
    ...
    def formulate_from_contexts(
        ...
        *,
        ...
        for operation in inbound_operations:
            ...
            transport_demand = need.transport_unit_count or need.quantity
            robot_start_nodes = {
                robot_id: robot_by_id[robot_id].current_node
                for robot_id in included_robot_ids
                if robot_id in robot_by_id
                and robot_by_id[robot_id].capacity_units >= transport_demand
            }
    ...
    *,
    ...
    if requested > included_robot_count:
        errors.append(
            ...
            + str(requested)
            + ";included="
            + str(included_robot_count)
        )
    actionable_work_count = len(draft.tasks) + len(draft.g2p_order_ids)
    if requested > actionable_work_count:
        errors.append(
            "MINIMUM_VEHICLE_COUNT_EXCEEDS_ACTIONABLE_WORK:requested="
            + str(requested)
            + ";operations="
            + str(actionable_work_count)
        )
    return errors
...
class CuOptDynamicInputValidator:
    ...
    def validate(
        ...
        *,
        ...
        inbound_slot_keys = [
            (str(task.rack_id), int(task.rack_level))
            for task in inbound_tasks
            if task.rack_id is not None and task.rack_level is not None
        ]
        if len(inbound_slot_keys) != len(set(inbound_slot_keys)):
            errors.append("DUPLICATE_PUTAWAY_SLOT")
        errors.extend(
        ...
        for task in inbound_tasks:
            ...
            expected_quantity = int(
                inbound_node.attributes.get("transport_unit_count")
                or inbound_node.attributes.get("quantity", 0)
            )
        ...
        if (
            _objective_is_explicit(normalized_request)
            and draft.objective_profile != normalized_request.constraints.objective_profile
        ):
            errors.append("OBJECTIVE_PROFILE_MISMATCH")
        if _objective_is_explicit(normalized_request) and normalized_request.constraints.objective_terms:
    ...
    def validate_from_contexts(
        ...
        *,
        ...
        for task in inbound_tasks:
            ...
            expected_pickups = {
                repository.mobile_handoff_node_for_inbound_access(str(value))
                for value in (handoff or {}).get("access_node_ids", [])
            }
            if not handoff or task.pickup_node not in expected_pickups:
                errors.append(f"INBOUND_PICKUP_MISMATCH:{task.order_id}")
            if task.delivery_node not in set(slot.access_node_ids):
                errors.append(f"INBOUND_DELIVERY_MISMATCH:{task.order_id}")
            expected_demand = need.transport_unit_count or need.quantity
            if task.item_id != need.item_id or task.demand != expected_demand:
                errors.append(f"INBOUND_ITEM_OR_QTY_MISMATCH:{task.order_id}")
        ...
        for robot_id in draft.fleet.included_robot_ids:
            ...
            vehicles.append(
                OptimizationVehicle(
                    ...
                    end_node=(
                        str(node.attributes["home_node"])
                        if node.attributes.get("home_node")
                        else None
                    ),
                    terminal_policy=(
                        "CHARGE" if node.attributes.get("home_node") else "STAY"
                    ),
        ...
        *,
        ...
        vehicles = [
            OptimizationVehicle(
                ...
                end_node=robot_by_id[robot_id].home_node,
                terminal_policy=(
                    "CHARGE" if robot_by_id[robot_id].home_node else "STAY"
                ),
