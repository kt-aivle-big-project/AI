            ...
            for step in planned.steps:
                ...
                station_reservations.append(
                    StationServiceReservation(
                        ...
                        end_at_ms=step.end_at_ms,
                        processed_quantity=batch.requested_quantity,
                        processing_ticks=batch.station_processing_ticks,
                    )
                )
                # One fixed station robot is shared by every physical access
                # spur.  Blocking all access nodes makes capacity=1 visible to
                # the next prioritized SIPP route without inventing AMR edges
                # between access A and B.
                for access_node in batch.station_access_node_ids or [batch.station_access_node]:
                    node_calendar.reserve(
                        edge_id=f"NODE:{access_node}",
