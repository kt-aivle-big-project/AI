...
from app.services.workload_routing_service import WorkloadRouteAssessment
...
def _has_agent_policy_stack(request: NormalizedWarehouseRequest) -> bool:
    ...
    return bool(
        ...
        or (
            ...
            and objective_terms
            and objective_terms != ["MIN_TRAVEL_DISTANCE"]
        )
    )
...
def resolve_request_gate(
    *,
    ...
    authoritative_structured_input: bool = False,
    planning_mode: str,
    ...
    workload_assessment: WorkloadRouteAssessment | None = None,
) -> RequestGateDecision:
    ...
    complete_authoritative_operations = bool(
        authoritative_structured_input
        and request.operations
        and not invalid_operations
        and all(
            operation.operation_type not in {"UNKNOWN"}
            for operation in request.operations
        )
    )
    ...
    workload_band = workload_assessment.band if workload_assessment is not None else "UNSPECIFIED"
    if planning_mode == "llm_router" and pure_structured and workload_band in {"UNSPECIFIED", "LOW"}:
    ...
    if planning_mode == "force_agent":
        ...
        route = "AGENT_FORMULATION"
        reasons.append(
            "A non-empty user_command requires Agent semantic formulation before deterministic validation."
        )
    elif workload_band == "HIGH":
        route = "AGENT_FORMULATION"
        reasons.append(
            workload_assessment.reason
            if workload_assessment is not None
            else "High workload requires Agent formulation."
        )
