...
from app.infrastructure.be_centered_postgres import BeCenteredDataError, BeCenteredPostgresAdapter
...
def health(request: Request) -> dict[str, object]:
    ...
    return {
        ...
        "native_plan_endpoints": [
            "GET /api/v1/simulation-runs/{simulation_run_id}/missions/plan/preflight",
            "POST /api/v1/simulation-runs/{simulation_run_id}/missions/plan",
            "POST /api/v1/simulation-runs/{simulation_run_id}/missions/replan",
        ...
        "be_centered_contract": {
            "operations": "request.structured_input",
            "orders_table": "not_used",
            "handling_units_table": "not_used",
            "inventory": "public.warehouse_items",
            "runtime": "Spring Redis simulation:run:{id}:*",
            "map": "Shared Neo4j RouteNode/TRAVERSES written by Spring BE",
        },
        "be_compat_endpoints": [
            "POST /optimize",
            "POST /reoptimize",
...
def list_warehouses() -> dict[str, object]:
    ...
    if settings.warehouse_repository_backend == "json":
        ...
        try:
            if settings.warehouse_repository_backend == "be_shared":
                records = BeCenteredPostgresAdapter().list_warehouses()
            else:
...
def bootstrap_infrastructure(
    ...
    if settings.warehouse_repository_backend == "be_shared":
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=(
                "Native JSON bootstrap is disabled in be_shared mode. Start Spring BE, "
                "then run scripts/prepare_be_centered_data.py for the numeric warehouse."
            ),
        )
    try:
...
def be_centered_plan_preflight(
    ...
    """Check BE tables, Spring Redis runtime, and the BE-derived Neo4j map."""
...
def generate_be_centered_fulfillment_commands(
    ...
    """Let an Agent choose a feasible BOX batch from live BE warehouse facts."""
...
def create_be_centered_simulation_plan(
    ...
    """Plan authoritative structured operations for an existing Spring run."""
...
def native_plan_preflight(
    ...
    if get_settings().warehouse_repository_backend == "be_shared":
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=(
                "be_shared mode requires the numeric Spring simulation-run preflight: "
                "GET /api/v1/simulation-runs/{id}/missions/plan/preflight."
            ),
        )
    try:
...
def create_simulation_plan(request: PublicMissionRequest) -> SimulationPlanResponse:
    ...
    if get_settings().warehouse_repository_backend == "be_shared":
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=(
                "be_shared mode does not read native orders/handling_units fixtures. "
                "Call POST /api/v1/simulation-runs/{id}/missions/plan with "
                "structured_input instead."
            ),
        )
...
def be_centered_replan(
    ...
    """Replan one Spring run and persist the pending plan in PostgreSQL."""
