...
class BeSpringRuntimeRepository:
    ...
    def bootstrap(
        ...
        *,
        ...
        """Create exact Spring-compatible Redis documents for local testing.

        This is not called by the Spring application.  It is exposed only via a
        gated compatibility debug endpoint and lets a developer test
        ``/reoptimize`` before the Spring simulation playback has been started.
        """

        robots = [
            value if isinstance(value, BeRobotStateInput) else BeRobotStateInput.model_validate(value)
            for value in robots
        ]
        if replace:
            old_ids = self._robot_ids(run_id)
            keys = [self.robot_state_key(run_id, value) for value in old_ids]
            keys += [self.robot_extension_key(run_id, value) for value in old_ids]
            keys += [self.robot_ids_key(run_id), self.meta_key(run_id)]
            if keys:
        ...
        for robot in robots:
            ...
            self.client.set(
                self.robot_state_key(run_id, robot.robot_id),
                json.dumps(state, ensure_ascii=False),
            )
            self.client.sadd(self.robot_ids_key(run_id), str(robot.robot_id))
        self.client.set(
