...
class Settings(BaseSettings):
    ...
    g2p_distinct_robot_per_handling_unit: bool = Field(
        ...
        description=(
            "When enough eligible AMRs exist, pre-assign one different AMR to each "
            "handling-unit cycle in the same outbound wave. If robots are fewer than "
            "cycles, remaining cycles stay solver-assignable and may run sequentially."
        ),
    )
    ...
    warehouse_repository_backend: str = Field(
        ...
        description="json uses fixtures; embedded uses local SQLite files; live connects to PostgreSQL, Redis, and Neo4j services.",
    )
    ...
    def normalize_warehouse_repository_backend(cls, value: object) -> str:
        ...
        if text not in {"json", "embedded", "live"}:
            raise ValueError("WAREHOUSE_REPOSITORY_BACKEND must be json, embedded, or live.")
        return text
