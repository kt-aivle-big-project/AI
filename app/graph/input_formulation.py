    ...
    merged = request.model_copy(
        update={
            ...
            "normalization_summary": (
                ...
                + " Authoritative structured event identifiers were preserved deterministically."
            ),
        }
    )
...
def request_router_llm_node(state: LaroGraphState) -> dict:
    ...
    try:
        ...
        routed = gateway.invoke_structured(
            system_prompt=REQUEST_ROUTER_SYSTEM,
            user_payload={
            ...
            output_model=RoutedNormalizedWarehouseRequest,
        ...
        summary = llm_summary(
            ...
            output_summary=(
                ...
                f"recommended={routed.recommendation.route}, gate={gate.action}, "
