...
2. after solver assignment, append a same-AMR return goal only when stock remains.
...
@observe_node(
    ...
    purpose=(
        "잔량 BOX에만 같은 AMR의 원선반 반환 Goal을 추가하고, "
        "소진 BOX는 고정 출고 설비 인계 지점에서 Cycle을 종료"
    ),
)
def goods_to_person_execution_enricher_node(state: LaroGraphState) -> dict:
    ...
    try:
        ...
        for batch in compilation.batches:
            ...
            demand.append(0)
            priorities.append(0)
            service_times.append(settings.handling_unit_return_service_ms)
        ...
        enrichment = GoodsToPersonRouteEnrichmentResult(
            ...
            batch_robot_assignments=batch_to_robot,
            warnings=[
                "Positive-remainder return goals are execution-only; depleted BOXes end at the fixed-station handoff."
            ],
        )
        return {
