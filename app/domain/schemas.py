...
class PublicMissionRequest(StrictModel):
    ...
    def validate_payload_shape(self) -> "PublicMissionRequest":
        infer_request_mode(events=list(self.events), user_command=self.user_command)
        return self
...
class OptimizationRequest(StrictModel):
    ...
    ``minimum_vehicle_count`` is an optional hard lower bound used by G2P
    waves when physically distinct handling units should leave their source
    zones in parallel. ``max_g2p_cycles_per_vehicle`` is independently checked
    after the solver and is enforced by the local OR-Tools backend.
    """
...
class StationServiceReservation(StrictModel):
    """Exclusive use of one fixed outbound-station robot."""
...
GoodsToPersonDisposition = Literal["RETURN_TO_HOME", "MOVE_TO_EMPTY_TOTE_BUFFER"]
GoodsToPersonPostStationAction = Literal["RETURN_TO_SOURCE", "MOVE_TO_EMPTY_TOTE_BUFFER"]
...
class HandlingUnitBatchPlan(StrictModel):
    ...
    def validate_quantities(self) -> "HandlingUnitBatchPlan":
        ...
        expected_disposition = (
            "RETURN_TO_HOME" if self.return_required else "MOVE_TO_EMPTY_TOTE_BUFFER"
        )
        expected_action = (
            "RETURN_TO_SOURCE" if self.return_required else "MOVE_TO_EMPTY_TOTE_BUFFER"
        )
        if self.disposition != expected_disposition:
            raise ValueError(f"disposition must be {expected_disposition}")
        if self.post_station_action != expected_action:
            raise ValueError(f"post_station_action must be {expected_action}")
        if self.return_required and self.post_station_node != self.source_access_node:
            raise ValueError("positive remainders must return to the source access node")
        if not self.return_required and not self.empty_tote_buffer_id:
            raise ValueError("depleted handling units require an empty tote buffer")
        expected_station = (
...
class InventoryMutationPreview(StrictModel):
    ...
    quantity_after: int = Field(ge=0)
    next_status: Literal[
        "stored", "at_station", "returning", "empty_in_transit", "empty_buffered"
    ]
    home_rack_id: str
