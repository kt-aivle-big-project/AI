...
def _low_battery_problem() -> tuple[dict, str, str]:
    nodes, edges = _warehouse_graph()
    work_id = "a84b946f-7a60-494a-ac9b-062ba7116d41"
    pick_id = f"{work_id}:1:pick"
    drop_id = f"{work_id}:1:drop"
    problem = {
        "warehouse_id": 2,
        "reference_time": "2026-07-27T00:00:09.660110+00:00",
        "captured_at": "2026-07-27T00:00:09.660110+00:00",
        "time_step_seconds": 5,
        ...
        "active_plan": {
            ...
            "reference_time": "2026-07-26T14:21:29.660110+00:00",
            "candidate_plan": True,
            "cuopt_plan": {
                "scheduled_tasks": [
                    {
    ...
    return problem, pick_id, drop_id


def test_cuopt_keeps_changeable_local_replan_robot_bound_but_unfrozen() -> None:
    problem, pick_id, drop_id = _low_battery_problem()

    normalized = _apply_assignments(problem, {"R2-03": [pick_id, drop_id]})
    rows = {row["task_id"]: row for row in normalized["tasks"]}

    assert rows[pick_id]["assigned_robot_id"] == "R2-03"
    assert rows[drop_id]["assigned_robot_id"] == "R2-03"
    assert rows[pick_id]["frozen"] is False
    assert rows[drop_id]["frozen"] is False
    assert normalized["cuopt_assignment_application"][
        "changeable_robot_bound_task_ids"
    ] == [pick_id, drop_id]
    assert normalized["cuopt_assignment_application"]["fixed_task_ids"] == []


def test_changeable_cuopt_result_allows_charge_before_pick_and_drop() -> None:
    problem, pick_id, drop_id = _low_battery_problem()
    normalized = _apply_assignments(problem, {"R2-03": [pick_id, drop_id]})
    ...
    assert [row.action for row in rows] == ["CHARGE", "PICK", "DROP"]
