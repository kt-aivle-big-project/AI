...
def run_pytest(paths: list[str], *, timeout: int = 120) -> int:
    command = [
        sys.executable,
        "-m",
        "pytest",
...
def main() -> int:
    ...
    parser.add_argument(
        "--full",
        action="store_true",
        help="Run the complete project regression after focused Gate 2.3 checks.",
    )
    ...
    print("Gate 2.3 focused regression")
    result = run_pytest(focused)
    if result:
        return result

    if not args.full:
        print("Gate 2.3 focused result: 30 passed / 0 failed")
    ...
    if count is None:
        print("Gate 2.3 full result: all batches passed / 0 failed")
    else:
        print(f"Gate 2.3 full result: {count} passed / 0 failed")
    return 0
