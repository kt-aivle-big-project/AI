...
def main() -> int:
    ...
    parser.add_argument(
        "--full",
        action="store_true",
        help="Run the complete project regression after focused P16.5.15.2 checks.",
    )
    ...
    print("P16.5.15.2 focused regression")
    result = run_pytest(focused)
    if result:
        return result
    print("P16.5.15.2 focused result: 97 passed / 0 failed")
    ...
    for index, batch in enumerate(balanced_batches(counts), start=1):
        ...
        if result:
            return result
    print(
        "P16.5.15.2 full result: all batches passed / 0 failed"
        if count is None
        else f"P16.5.15.2 full result: {count} passed / 0 failed"
    )
    return 0
