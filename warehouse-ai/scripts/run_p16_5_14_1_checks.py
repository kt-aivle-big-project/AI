...
def main() -> int:
    ...
    parser.add_argument(
        "--full",
        action="store_true",
        help="Run the complete project regression after focused P16.5.14.1 checks.",
    )
    ...
    print("P16.5.14.1 focused regression")
    result = run_pytest(focused)
    if result:
        return result

    if not args.full:
        print("P16.5.14.1 focused result: 78 passed / 0 failed")
    ...
    if count is None:
        print("P16.5.14.1 full result: all batches passed / 0 failed")
    else:
        print(f"P16.5.14.1 full result: {count} passed / 0 failed")
    return 0
