...
def main() -> int:
    ...
    parser.add_argument(
        "--full",
        action="store_true",
        help="Run the complete project regression after the focused Gate 2.2 checks.",
    )
    ...
    focused = [
        ...
        "tests/test_p16_5_5_multi_robot_rebalance.py",
    ...
    print("Gate 2.2 focused regression")
    result = run_pytest(focused)
    if result:
        return result

    if not args.full:
        print("Gate 2.2 focused result: 47 passed / 0 failed")
    ...
    if count is None:
        print("Gate 2.2 full result: all batches passed / 0 failed")
    else:
        print(f"Gate 2.2 full result: {count} passed / 0 failed")
    return 0
