        ...
        if tool == "find_orders":
            ...
            for receipt in receipts:
                ...
                inbound_movements.append({
                    ...
                    "facility_access_node_ids": (
                        [str(value) for value in handoff.get("access_node_ids", [])]
                        if handoff else []
                    ),
                    "pickup_access_node_ids": (
                        [
                            self.repository.mobile_handoff_node_for_inbound_access(str(value))
            ...
            canonical_ids = [
                ...
                *[
                    self.repository.mobile_handoff_node_for_inbound_access(str(access_node_id))
                    for value in handoffs
                    for access_node_id in value.get("access_node_ids", [])
                ],
        ...
        if orders and g2p_mode:
            ...
            station_accesses = [
                (
                    ...
                    str(access_node_id),
                    self.repository.mobile_handoff_node_for_station_access(
                        str(access_node_id)
                    ),
                    sorted(
                        {
                        ...
                        }.intersection(logical_destinations)
                    ),
                )
            ...
            # Fixed-station access nodes are outside the AMR driving graph.
            # Agent path evidence must stop at the mobile-side handoff.
            anchors.update(handoff for _, _, handoff, _ in station_accesses)
            ...
            for station_id, station_access, mobile_handoff, _ in station_accesses:
                for buffer_id, buffer_access in empty_accesses:
                    key = (mobile_handoff, buffer_access, buffer_id)
        ...
        inbound_needs = [
            InboundTaskNeed(
                ...
                transport_unit_count=(
                    int(value["transport_unit_count"])
                    if value.get("transport_unit_count") is not None
                    else None
                ),
