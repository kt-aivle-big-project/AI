...
class JsonWarehouseRepository:
    ...
    def _validate_references(self) -> None:
        ...
        for rack_id, access_node_ids in self.rack_access_by_rack.items():
            self._validate_rack_access_contract(rack_id, access_node_ids)
        # Inbound handoff and outbound station access nodes are service-only
        # endpoints.  Native maps may expose two access nodes per facility,
        # while maps authored by the BE layout editor may collapse the same
        # facility to one endpoint with one or more aisle neighbours.
        for collection_name, collection, node_type, id_field, allowed_counts in (
            ("inbound handoff", self.inbound_handoffs, "inbound_handoff_access", "handoff_id", {1, 2}),
            ("outbound station", self.outbound_stations, "outbound_station_access", "station_id", {1, 2, 3, 4}),
            ...
            for resource_id, resource in collection.items():
                ...
                for access_node_id in access_node_ids:
                    ...
                    route_boundary = (
                        collection_name == "outbound station"
                        and node.get("type") == "route"
                        and node.get("transit_allowed", True) is not False
                    )
                    if not route_boundary and (
                        node.get("type") != node_type
                        or str(node.get(id_field)) != resource_id
                    ):
                        raise DataContractError(
                            f"access node {access_node_id} is not a {node_type} for {resource_id}"
                        )
                    if route_boundary:
                        # A fixed outbound robot may expose up to four ordinary
                        # AMR boundary nodes. They stay in the aisle graph and
                        # are reserved as station hand-off ports by MAPF.
                        continue
                    ...
                    if not peer_nodes:
                        raise DataContractError(
                            f"service access node {access_node_id} must connect to at least one route node"
                        )
                    if any(
                        str(self.nodes[peer].get("type") or "").endswith("_access")
                        for peer in peer_nodes
                    ):
                        raise DataContractError(
                            f"service access node {access_node_id} must not connect to another service access node"
                        )
                    if not any(
                        self.nodes[peer].get("transit_allowed", True) is not False
                        for peer in peer_nodes
                    ):
                        raise DataContractError(
                            f"service access node {access_node_id} must connect to a transit-allowed route node"
                        )
    ...
    def _validate_rack_access_contract(
        ...
        """Validate the native LARO rack-access routing contract.

        Shared BE maps override this hook because their physical rack node is
        connected directly to two ordinary aisle nodes instead of two
        synthetic ``rack_access`` dead-end nodes.
        """
        ...
        *,
        ...
        def type_allowed(entity_type: str) -> bool:
            ...
            if upper == "INBOUND" and "NODE" in expected:
                return True
            # Layout-editor facility nodes are canonical map nodes too.  The
            # BE-centered graph stores the physical endpoint type, while the
            # retrieval contract asks for the broader semantic entity type.
            # Exact IDs such as IN_HANDOFF_1_ACCESS_A and OUT_STATION_1 must
            # therefore survive this type boundary.
            if upper == "INBOUND_HANDOFF_ACCESS" and expected.intersection(
                {"NODE", "INBOUND", "INBOUND_HANDOFF"}
            ):
                return True
            if upper == "OUTBOUND_STATION_ACCESS" and expected.intersection(
                {"NODE", "OUTBOUND", "OUTBOUND_STATION"}
            ):
                return True
            if upper == "EMPTY_TOTE_BUFFER_ACCESS" and expected.intersection(
                {"NODE", "EMPTY_TOTE_BUFFER"}
            ):
                return True
    ...
    def outbound_station_candidates(self, chute_ids: list[str]) -> list[dict[str, Any]]:
        """Return stations that cover at least one requested destination.

        Native G2P maps model logical chutes separately.  The BE layout editor
        may instead use each outbound station endpoint itself as the logical
        destination.  A station therefore always serves its own canonical ID
        in addition to any explicitly configured chute IDs.

        This repository method builds planning context for an entire wave, so
        it must return the union of relevant stations. Requiring one station to
        cover every destination incorrectly removes all candidates when a BE
        request contains OUT_STATION_1 and OUT_STATION_2 together. The physical
        handling-unit compiler performs its stricter all-destinations check for
        ...
        for station in self.outbound_stations.values():
            ...
            if not required or required.intersection(coverage):
    ...
    def mobile_handoff_node_for_inbound_access(self, access_node_id: str) -> str:
        """Resolve the AMR-side route node for an inbound handoff facility.

        The inbound access node represents the fixed receiving facility.  The
        mobile robot stops on its adjacent route node and receives the box
        across the facility boundary.  Older snapshots without the explicit
        ``adjacent_route_node`` property fall back to the connected route peer.
        """

        access_id = str(access_node_id)
        access_node = self.nodes.get(access_id, {})
        adjacent = access_node.get("adjacent_route_node")
        if adjacent and str(self.nodes.get(str(adjacent), {}).get("type")) == "route":
            return str(adjacent)
        ...
        for edge in self.edges.values():
            ...
            peer = target if source == access_id else source if target == access_id else None
            if peer and str(self.nodes.get(peer, {}).get("type")) == "route":
                candidates.append(peer)
        if not candidates:
            return access_id

        access_y = float(access_node.get("y", 0.0) or 0.0)
        return min(
            set(candidates),
            key=lambda node_id: (
                abs(float(self.nodes.get(node_id, {}).get("y", 0.0) or 0.0) - access_y),
                node_id,
            ),
        )

    def mobile_handoff_node_for_station_access(self, access_node_id: str) -> str:
        """Resolve the AMR-side boundary node for a fixed outbound station.

        Outbound station access nodes belong to the fixed station robot.  A
        mobile warehouse robot must stop on the adjacent route node and hand
        the handling unit across that boundary.  The edge-based fallback keeps
        older graph snapshots usable when ``adjacent_route_node`` is absent.
        """

        access_id = str(access_node_id)
        access_node = self.nodes.get(access_id, {})
        if (
            str(access_node.get("type")) == "route"
            and access_node.get("transit_allowed", True) is not False
        ):
            return access_id
        def peer_for(edge: dict[str, Any], node_id: str) -> str | None:
            source = str(edge.get("source"))
            target = str(edge.get("target"))
            return target if source == node_id else source if target == node_id else None

        def station_service_edge(edge: dict[str, Any]) -> bool:
            return bool(edge.get("service_only")) or str(
                edge.get("type") or edge.get("edge_type") or ""
            ).casefold() in {"outbound_service", "station_service"}

        adjacent = access_node.get("adjacent_route_node")
        station_route_candidates: list[str] = []
        if adjacent and str(adjacent) in self.nodes:
            station_route_candidates.append(str(adjacent))
        for edge in self.edges.values():
            peer = peer_for(edge, access_id)
            if peer and str(self.nodes.get(peer, {}).get("type")) == "route":
                station_route_candidates.append(peer)

        access_y = float(access_node.get("y", 0.0) or 0.0)
        station_route_candidates = sorted(
            set(station_route_candidates),
            key=lambda node_id: (
                abs(float(self.nodes.get(node_id, {}).get("y", 0.0) or 0.0) - access_y),
                node_id,
            ),
        )
        if not station_route_candidates:
            return access_id

        station_route = station_route_candidates[0]
        # The edited BE map has two shared blue station hubs (N166/N167).
        # Each hub serves several fixed station endpoints.  Those hubs belong
        # to the fixed robot/conveyor workspace, so an AMR must stop one lane
        # earlier on the purple route boundary.  Older maps have one private
        # access spur per station; keep their adjacent route node unchanged.
        fixed_endpoint_ids: set[str] = set()
        for edge in self.edges.values():
            peer = peer_for(edge, station_route)
            if not peer or not station_service_edge(edge):
                continue
            if str(self.nodes.get(peer, {}).get("type")) == "outbound_station_access":
                fixed_endpoint_ids.add(peer)
        if len(fixed_endpoint_ids) < 2:
            return station_route

        boundary_candidates: list[str] = []
        for edge in self.edges.values():
            peer = peer_for(edge, station_route)
            if not peer or station_service_edge(edge):
                continue
            if edge.get("mobile_robot_traversable") is False:
                continue
            if str(self.nodes.get(peer, {}).get("type")) == "route":
                boundary_candidates.append(peer)
        return min(
            set(boundary_candidates),
            key=lambda node_id: (
                abs(float(self.nodes.get(node_id, {}).get("y", 0.0) or 0.0) - access_y),
                node_id,
            ),
            default=station_route,
        )
    ...
    def inbound_handoff_for_port(self, port_id: str) -> dict[str, Any] | None:
        ...
        handoff_id = str(port.get("handoff_id")) if port else str(port_id)
        value = self.inbound_handoffs.get(handoff_id)
        return dict(value) if value else None
    ...
    def adjusted_arcs(
        ...
        *,
        ...
        # Shared blue outbound hubs belong to fixed station robots, not the
        # mobile-robot road graph.  The BE editor currently persists their
        # purple-to-blue connector as an ordinary lane; infer the physical
        # boundary from the hub's multiple outbound-service endpoints so an
        # AMR can neither occupy the hub nor use it as a shortcut between rows.
        fixed_station_endpoints: dict[str, set[str]] = {}
        for edge in self.edges.values():
            if not (
                bool(edge.get("service_only"))
                or str(edge.get("type") or edge.get("edge_type") or "").casefold()
                in {"outbound_service", "station_service"}
            ):
                continue
            source = str(edge.get("source"))
            target = str(edge.get("target"))
            for candidate, peer in ((source, target), (target, source)):
                if (
                    str(self.nodes.get(candidate, {}).get("type")) == "route"
                    and str(self.nodes.get(peer, {}).get("type"))
                    == "outbound_station_access"
                ):
                    fixed_station_endpoints.setdefault(candidate, set()).add(peer)
        fixed_station_hubs = {
            node_id
            for node_id, endpoint_ids in fixed_station_endpoints.items()
        ...
        for edge_id, edge in self.edges.items():
            if edge_id in blocked_edge_ids:
                continue
            if str(edge["source"]) in fixed_station_hubs or str(edge["target"]) in fixed_station_hubs:
                continue
            ...
            arcs.append(
                {
                    ...
                    "source_service_only": self.nodes[str(edge["source"])].get("service_only") is True,
                    "target_service_only": self.nodes[str(edge["target"])].get("service_only") is True,
                }
            )
        return arcs
...
def _create_repository(
    ...
    if settings.warehouse_repository_backend == "be_shared" and override is None:
        raise DataContractError(
            "WAREHOUSE_REPOSITORY_BACKEND=be_shared requires the numeric Spring "
            "simulation-run endpoint: POST /api/v1/simulation-runs/{id}/missions/plan. "
            "That endpoint creates a request-scoped BeSharedWarehouseRepository and "
            "does not read JSON order/handling-unit fixtures."
        )
