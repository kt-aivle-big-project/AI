...
class Neo4jMapRepository:
    ...
    def validate_snapshot(
        ...
        """Reject rack transit nodes and malformed service-access topology.

        Rack access points remain physical dead-end spurs.  Facility endpoints
        (inbound handoffs, outbound stations, and empty-tote buffers) may expose
        more than one aisle connection when the BE layout editor represents the
        logical facility as a single node.  They are still service-only and may
        never connect directly to another service endpoint.
        """
        ...
        for node_id, node in node_by_id.items():
            ...
            if not peers:
                raise Neo4jMapRepositoryError(
                    f"Service access node {node_id} must connect to at least one route node."
                )
            if node_type == "rack_access" and len(peers) != 1:
                raise Neo4jMapRepositoryError(
                    f"Rack access node {node_id} must be a one-neighbour dead-end spur."
                )
            if any(
                str(node_by_id[peer].get("type") or "") in service_access_types
                for peer in peers
            ):
                raise Neo4jMapRepositoryError(
                    f"Service access node {node_id} must not connect to another service access node."
                )
            if not any(node_by_id[peer].get("transit_allowed", True) is not False for peer in peers):
                raise Neo4jMapRepositoryError(
                    f"Service access node {node_id} must connect to a transit-allowed route node."
                )
